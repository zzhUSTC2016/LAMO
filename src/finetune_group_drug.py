...
# torchrun --nproc_per_node=8 --master_port=29501 finetune.py --learning_rate 5e-5 --num_epochs 1 --train_on_inputs 0 --resume_from_checkpoint 1 --data_path ../data/finetune_data_ratio_7.json --output_dir ../output/ratio_7_5e-5_input/  > ratio7_input_5e-5_continue.out 2> ratio7_input_5e-5_continue.err
...
# CUDA_VISIBLE_DEVICES=3,4 nohup torchrun --nproc_per_node=2 --master_port=29502 finetune.py --data_path ../data/finetune_data/Acetaminophen-Potassium-chloride-Pantoprazole-Metoprolol-Magnesium-sulfate.json --output_dir ../output/tmp/ --eval_epochs 4 --num_epochs 20   --early_stopping_patience 15 --val_set_size 3020 > test.out 2> test.err &
...
def train(
    ...
    cutoff_len: int = 5120,
    patient_num: int = 200,   # 每个药物的病人数
    val_set_size: int = 1000, # 这一组药物的验证集大小
    ...
    class MyEarlyStoppingCallback(EarlyStoppingCallback):
        ...
        def on_evaluate(self, args, state, control, metrics, **kwargs):
            ...
            if state.global_step > 640 and self.early_stopping_patience_counter >= self.early_stopping_patience:
                            #  and state.best_metric > random_jaccard:
    ...
    def metrics_per_drug(preds, labels):
        ...
        return recall, precision, f1, jaccard, num_drugs, num_drugs_gt

    
    def metrics(preds, labels):
        recall_list, precision_list, f1_list, jaccard_list, \
            num_drugs_list, num_drugs_gt_list = [], [], [], [], [], []
        val_drug_num = val_set_size // patient_num
        for i in range(val_drug_num):
            start = i * patient_num
            end = (i + 1) * patient_num
            recall, precision, f1, jaccard, num_drugs, num_drugs_gt = metrics_per_drug(
    ...
    def compute_metrics(eval_preds):
        ...
        recall, precision, f1, jaccard, num_drugs, num_drugs_gt = metrics(preds, labels)
        return {
            "num_drugs": num_drugs,
